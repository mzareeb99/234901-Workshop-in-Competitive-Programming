...
void getNumebrZero(vsi& vs, int &zeroIndegree,int &zeroOutdegree){
    vector<int> zero(vs.size(), true);
    for (auto& v : vs)
        for (auto& u : v)
            zero[u] = 0;
    int ret = 0;
...
int main() {
    ...
    while (T--) {
        ...
        int zero_indegree,zero_outdegree;
        getNumebrZero(scc,zero_indegree,zero_outdegree);
