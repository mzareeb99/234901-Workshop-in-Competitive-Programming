...
int main() {
   ...
   while(cin>>N){
       ...
       for(int i =0; i < N;i++){
           ...
           while(k >0 &&vec[i].first - vec[k].first < min_x){
               k--;
               min_x= min(min_x, max(abs(vec[i].first - vec[k].first),
                                     abs(vec[i].second-vec[k].second)));
           }
       }
...
//       });
 ...
 //      int min_y=MAX_SIZE;
//       for(int i =0; i < N-1;i++){
//           if(vec[i+1].second -vec[i-1].second < min_y)
//               min_y  = vec[i+1].second -vec[i-1].second;
//       }
       //int ret = min(min_x,min_y);
       cout<<min_x<<endl;
   }
