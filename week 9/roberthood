...
long double currMax(const vector<point>& ch, int i){
    auto left_bound = 0;
    auto right_bound = ch.size();
    while (right_bound - left_bound > 2){
        auto mid1 = left_bound + (right_bound - left_bound) / 3;
        auto mid2 = right_bound - (right_bound - left_bound) / 3;
        auto d1 = dist(ch[i], ch[(i + mid1) % ch.size()]);
        auto d2 = dist(ch[i], ch[(i + mid2) % ch.size()]);
        if (d1 < d2){
            left_bound = mid1;
        } else{
            right_bound = mid2;
    ...
    long double curr=0;
    int j=(i+left_bound)%ch.size();
    curr = max(curr, dist(ch[i], ch[j]));
    j = (j+1)%ch.size();
    for (; j!=(i+right_bound)%ch.size(); j = (j+1)%ch.size()){
        curr = max(curr, dist(ch[i], ch[j]));
    }
    return curr;
}
...
int main() {
    ...
    for (int i=0; i<CH.size(); ++i){
        max_d = max(max_d, currMax(CH, i));
    }
    cout<<fixed<<setprecision(8)<<max_d<<endl;
    return 0;
}
